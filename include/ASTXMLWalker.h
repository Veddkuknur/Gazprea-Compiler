#ifndef ASTXMLWALKER_H
#define ASTXMLWALKER_H
...
public:
    ...
    void selfClosingTag(const std::string& tag_name);

    void addIndent();

    void addTypedef(const std::shared_ptr<ASTNode>& node);

    void addDeclLeft(bool is_constant, const std::shared_ptr<Type>& type_obj, const std::string& id);

    void addParameter(std::tuple<bool, std::shared_ptr<Type>, std::string> param);
    void addParameter(std::tuple<std::shared_ptr<Type>, std::string> param);
    void addReturnType(const std::shared_ptr<Type>& type_obj);

    void addInt(int val);
    void addReal(float val);
...
class ASTXMLWalker : public BaseASTWalker {
public:

    ~ASTXMLWalker() = default;
