#ifndef TYPEINFERWALKER_H
#define TYPEINFERWALKER_H

#include "BaseASTWalker.h"
class TypeInferWalker : public BaseASTWalker {
public:
    ~TypeInferWalker() override = default;

    std::any inferBoolVector(std::shared_ptr<Type> left_side, std::shared_ptr<Type> right_side, std::string op);
    ...
    void inferVectorTypeAtDecl(std::shared_ptr<DeclNode> &source_node);
    void inferDeclVectorWithoutSize(std::shared_ptr<DeclNode> &source_node);
    void inferDeclVectorWithSize(std::shared_ptr<DeclNode> &source_node);


    void inferVectorTypeAtTupleWithSize(std::shared_ptr<ASTNode> &expr_node, std::shared_ptr<Type> &source_type);
    void inferVectorTypeAtTupleWithoutSize(std::shared_ptr<ASTNode> &expr_node, std::shared_ptr<Type> &source_type);
    void inferVectorTypeAtTupl(std::shared_ptr<ASTNode> &expr_node, std::shared_ptr<Type> &source_type);
    Type::BaseType declPromMatrix(Type::BaseType from_type, Type::BaseType to_type);
};

#endif //TYPEINFERWALKER_H
