#ifndef CODEGENWALKER_H
#define CODEGENWALKER_H
...
class CodeGenASTWalker : public BaseASTWalker {
    /**
     * This class walks the tree and generates calls the Backend APIs
     * */
public:
    BackEnd backend;
    bool passing_mutable_id = false;

    mlir::Value vectorBinaryOp(mlir::Value left_side, mlir::Value right_side,
        std::string op, std::shared_ptr<Type> &type, int line);
    ...
    mlir::Value castVector(mlir::Value vector_addr, std::shared_ptr<Type> &vec_type, std::shared_ptr<Type> &to_type);
};

#endif //CODEGENWALKER_H
